...
import { Sun, Moon, Plus, MagnifyingGlass } from "@phosphor-icons/react"
    ...
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-[var(--border)] bg-[var(--card)] px-4 lg:px-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {subtitle && (
            <p className="mt-0.5 text-xs text-[var(--muted-foreground)] hidden sm:block">{subtitle}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--muted-foreground)] hover:bg-[var(--muted)] hover:text-[var(--foreground)] transition-colors">
